...
<div class="clearfix tabs-linearrow">
	<ul class="nav nav-tabs">
		<li class="{{ $user || count($errors)>0 ? '' : 'active' }}"><a href="{{url('config/user')}}" hreff="#tab-linearrow-one" -data-toggle="tab">CONSULTAS</a></li>
...
<!-- Data Table -->
							...
							<table class="table table-bordered table-striped">
								...
								<tbody>
									...
									<tr class="{{!$u->is_active ? 'text-disable----' : ''}}">
									    <td><a href="{{url("config/user/{$u->id}/edit")}}">{{$u->email}}</a></td>
									    <td>{{$u->name}}</td>
									    <!--<td>{{$u->roles->implode('name',',')}}</td>-->
									    <!--<td><a href="{{url("config/user/{$u->id}/status")}}">{{$u->is_active_slug()}}</a></td>-->
									    <td width="5%">
									        @if($u->canDelete())
									        <a href="{{url("config/user/{$u->id}")}}" data-method="delete" data-token="{{csrf_token()}}" data-confirm="Estas seguro?" class="md-fab md-primary md-button md-mini waves-effect"><i class="fa fa-remove"></i></a>
									        @endif
									    </td>
									</tr>
									@endforeach
		...
		<div class="tab-pane {{ $user || count($errors)>0  ? 'active' : '' }}" id="tab-create">
		    <form role="form" class="form-horizontal" method="POST" action="{{ url("config/user") }}{{$user ? "/{$user->id}" : ''}}">
		    {!! csrf_field() !!}
                ...
                <div class="row">
                    ...
                    <div class="col-md-2">
                        <div class="btn-group">
                            ...
                            @if($user)
                            <a href="{{url("config/user/{$user->id}")}}" data-method="delete"
